 ...
 * @LastEditors: tao
 * @LastEditTime: 2021-02-09 14:44:50
-->
<template>
  <div class="section">
    <div class="header">
    ...
    <div class="body">
      <div class="body_item" v-for="(item, i) in detail" :key="i">
        ...
        <div class="cell">
          <a-input v-model="item.note" placeholder="请输入备注" />
        </div>
...
export default {
  ...
  methods: {
    ...
    handleChange(i, value) {
      // console.log(i, value);
      this.detail[i].productId = value;
      this.detail[i].singlePrice = +this.products.filter(
        (item) => item.id == value
      )[0].price;
    },
    onChange(i) {
    ...
    getDetail() {
      return this.detail.map((item) => {
        ...
        item.sumPrice = item.singlePrice * item.count;
        return item;
      });
    },
    delDetailItem(i) {
